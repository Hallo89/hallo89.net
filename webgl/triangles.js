...
void main() {
    ...
    gl_Position = vec4(position, 0, 1) + vec4(offset, 0, 0);
}
`;
...
gl.clearColor(23/255, 70/255, 158/255, 1);
...
for (i = 0; i < 20; i++) {
  for (u = 0; u < 20; u++) {
    ...
    for (n = 0; n < 2; n++) {
      ...
      gl.uniform2fv(offsetPosition, [0.1 * u, -0.1 * i]);
  	  let lightness = (Math.random() + 0.25) * 2 * (u + i + 10) / 40;
      gl.uniform4fv(colorPosition, [
        (23/255 * lightness).toFixed(2),
        (70/255 * lightness).toFixed(2),
        (158/255 * lightness).toFixed(2),
        1
      ]);
